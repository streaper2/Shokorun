...
function _levelSelect:draw()
    ...
    if  rOn ==  false and lOn  == false then
...
function _levelSelect:controller(key)
    ...
    --selector move
    if key == "right" and selector.val < #_levelSelect.lvlFiles and lOn == false and rOn == false and not love.keyboard.isDown( 'lshift' ) then
    ...
    if key == "left" and lOn == false and rOn == false and not love.keyboard.isDown( 'lshift' )  then
